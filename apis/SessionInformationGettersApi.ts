...
export class SessionInformationGettersApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async phoneIdContactConversationIdGet(phoneId: string, conversationId: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'phoneId' is not null or undefined
        if (phoneId === null || phoneId === undefined) {
            throw new RequiredError("SessionInformationGettersApi", "phoneIdContactConversationIdGet", "phoneId");
        }


        // verify required parameter 'conversationId' is not null or undefined
        if (conversationId === null || conversationId === undefined) {
            throw new RequiredError("SessionInformationGettersApi", "phoneIdContactConversationIdGet", "conversationId");
        }


        // Path Params
        const localVarPath = '/{phone_id}/contact/{conversation_id}'
...
export class SessionInformationGettersApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to phoneIdContactConversationIdGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async phoneIdContactConversationIdGet(response: ResponseContext): Promise<PhoneIdContactConversationIdGet200Response > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: PhoneIdContactConversationIdGet200Response = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "PhoneIdContactConversationIdGet200Response", ""
            ) as PhoneIdContactConversationIdGet200Response;
            return body;
        }
